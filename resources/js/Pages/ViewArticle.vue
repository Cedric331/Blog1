                ...
                <div v-for="(commentaire, index) in commentaires" :key="commentaire.id">
                    <div v-if="count > index" class="media d-block d-md-flex mt-3 shadow p-1 mb-2 bg-white rounded">
                        <div class="media-body text-center text-md-left ml-md-3 ml-0">
                            <em>Posté le: {{ commentaire.created_at }} <strong v-if="commentaire.created_at != commentaire.updated_at">- modifié</strong></em>
                            ...
                            <div v-if="commentEditFormVisible === commentaire.id" class="form-group shadow-lg p-3 mb-5 bg-light rounded">
                                <label for="commentaire">Modifier le commentaire: </label>
                                <textarea class="form-control bg-white border border-dark"
                                    id="commentaire" rows="3" spellcheck="false" data-gramm="false" v-model="commentEditVisible"
                        ...
                        <button v-if="user.id == commentaire.user.id" @click="formEditComment(commentaire)"
